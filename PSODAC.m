...
%% 参数初始化
c1_initial = 2;          % 初始自我认知参数
c2_initial = 2;          % 初始社会认知参数
Vmax = 2;                % 最大速度
Vmin = -2;               % 最小速度
popmax = 50;             % 位置最大值
popmin = 0;              % 位置最小值
gbest = zeros(n, 2);     % 全局最优解
...
for i = 1:sizepop
    pop(i).Position = rand(n, 2) * L;         % 初始粒子位置
    pop(i).V = randn(n, 2) * 2;              % 初始化粒子速度
    fitness(i) = fun(pop(i).Position(:, 1), pop(i).Position(:, 2), L, R, data);  % 适应度函数
end
...
%% 迭代寻优
for i = 1:maxgen
    W = Wmax - ((Wmax - Wmin) / maxgen) * i;  % 惯性权重更新
    c1 = c1_initial - (c1_initial - 0.5) * (i / maxgen);  % 自我认知参数动态减少
    c2 = c2_initial + (2.5 - c2_initial) * (i / maxgen);  % 社会认知参数动态增加
