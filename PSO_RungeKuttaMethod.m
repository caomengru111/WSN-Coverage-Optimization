...
for i = 1:n
    ...
    hold on;
    fill(x(i) + R*cos(sita), y(i) + R*sin(sita), 'y');
end
...
for i = 1:maxgen
    for j = 1:sizepop
        %% 使用龙格库塔法进行位置更新
        % 这里我们使用适应度值作为动力学方程的一部分
        DelX = rungeKutta(pop(j).Position, gbest); % 这里调用龙格库塔法
        pop(j).Position = pop(j).Position + DelX;   % 更新位置
...
for i = 1:n
    ...
    sita = 0:pi/100:2*pi;   % 角度[0, 2*pi]
    hold on;
    fill(x(i) + R*cos(sita), y(i) + R*sin(sita), 'g');
end
p1 = plot(gbest(:, 1), gbest(:, 2), 'r*');
legend([p1], {'WSNs节点'});
...
function DelX = rungeKutta(position, gbest)
    % 这里是简单的龙格库塔法实现
    % 根据您的需求，可以调整动态方程
    dt = 0.01; % 步长
    k1 = dt * (gbest - position);
    k2 = dt * (gbest - (position + 0.5 * k1));
    k3 = dt * (gbest - (position + 0.5 * k2));
    k4 = dt * (gbest - (position + k3));
    
    DelX = (k1 + 2 * k2 + 2 * k3 + k4) / 6; % 龙格库塔公式
end